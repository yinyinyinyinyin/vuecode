...
<script>
  //引入商品分类和商品品牌
  import {fetchListWithChildren, fetchBrandList, createProduct,fetchCategoryList,createProductCate } from '@/api/product';
	  ...
	  handleSubmit(formName){
		    ...
		    this.$refs[formName].validate((valid) => {
		      if (valid) {
				 ...
				 createProductCate(this.productCate).then(res=>{
					 console.log(res);
					 if(res.type === 'success'){
						this.$message({
						  message: '成功添加商品分类',
